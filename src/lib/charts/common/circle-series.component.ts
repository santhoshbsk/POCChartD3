...
@Component({
  selector: 'g[ana-charts-circle-series]',
  template: `
    <svg:g *ngIf="circle">
      <defs>
        <svg:g ana-charts-svg-linear-gradient
...
export class CircleSeriesComponent implements OnChanges, OnInit {
  ...
  /*
  *C-SK Circle to render for single measure line */
  getCircles(): any[] {
  ...
  getTooltipText({ tooltipLabel, value, seriesName, min, max }): string {
    if (seriesName && tooltipLabel && seriesName !== tooltipLabel) {
      return `
